...
#' @examples
#' #
calc_func <- function(df, added = NULL) {
  ...
  if (!is.null(added)) {
    colnames(added) <- colnames(herb_efficacy)
    herb_efficacy2 <- rbind(herb_property, added)
  } else {
    herb_efficacy2 <- herb_efficacy
  }

  herb_func <- herb_func <- left_join(herb_efficacy, sum_herb, by = 'names') %>%
    drop_na() %>%
