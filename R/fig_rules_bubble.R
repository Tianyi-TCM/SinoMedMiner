...
fig_rules_bubble <- function(rules, shading = "lift", measure = "support",
  ...
  # 修改标签
  if (measure == "support") {
    measure_lable <- "\u652f\u6301\u5ea6"  # Support
  } else if (measure == "confidence") {
    measure_lable <- "\u7f6e\u4fe1\u5ea6"  # Confidence
  } else {
    measure_lable <- "\u63d0\u5347\u5ea6"  # Lift
  }

  if (shading == "support") {
    shading_lable <- "\u652f\u6301\u5ea6"  # Support
  } else if (shading == "confidence") {
    shading_lable <- "\u7f6e\u4fe1\u5ea6"  # Confidence
  } else {
    shading_lable <- "\u63d0\u5347\u5ea6"  # Lift
  }


  q$labels$size = measure_lable  # Support
  q$labels$colour = shading_lable  # Lift
  q$labels$x = "\u524d\u9879"  # LHS
  q$labels$y = "\u540e\u9879"  # RHS
