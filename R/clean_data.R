...
clean_data <- function(df, lookup_table) {

  df_long <- trans_long(df)
  colnames(df_long) <- c("id", "itemnames")

  ##### Clean data: remove newline characters and leading/trailing spaces #####
  # Initialize flags
  has_newline <- FALSE
  herbs_with_spaces <- FALSE

  df_long[] <- lapply(df_long, function(x) {
    # Detect and remove newline characters
    if (any(grepl("\n", x))) {
      has_newline <<- TRUE
      x <- gsub("\n", "", x)
    }
    # Detect and remove leading/trailing spaces
    if (any(grepl("^\\s|\\s$", x))) {
  ...
  ##### Check for duplicate herbs within the same prescription. #####
  ...
  if (nrow(repeated_herbs) > 0) {
    # Create a detailed message showing the id and duplicated herb names
  ...
  #### Determine if the data contains dosage information. ####
  contains_numbers <- any(grepl("\\d", df_long$itemnames))

  # If dosage information is present, execute the following code.
  if (contains_numbers) {
    ...
    # Split herbs and dosage.
    df_long <- tidyr::separate(df_long, "itemnames", into = c("items", "graw_weight"), sep = "(?<=\\D)(?=\\d)")
