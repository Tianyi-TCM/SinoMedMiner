...
#' @export
#'
normalize_herb_names <- function(df, type = NULL) {
  ...
  herb_standard_names_path <- system.file("data", "herb_standard_names.rda", package = "TCMminer")
  load(herb_standard_names_path)
  herb_standard_names <- get("herb_standard_names")

  standard.herb.names <- unique(unlist(herb_standard_names))
