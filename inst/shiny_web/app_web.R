...
.align-bottom-row {
  ...
  min-height: 75px; /* 根据控件高度调整 */
}
.align-bottom-row .form-group {
  margin-bottom: 0;
  display: flex;
...
ui <- shinyUI(fluidPage(
  ...
  navbarPage(
    ...
    tabPanel("数据清洗",
             sidebarLayout(
               sidebarPanel(
                 fileInput("dataFile", "上传待清洗的数据文件 (.xlsx)", accept = ".xlsx", buttonLabel = "浏览"),
                 fileInput("lookupFile", "上传清洗规则文件 (.xlsx)", accept = ".xlsx", buttonLabel = "浏览"),
                 div(class = "btn-group-custom", # 使用自定义样式类
    ...
    tabPanel("中药属性",
             sidebarLayout(
               sidebarPanel(
                 useShinyjs(),
                 fileInput("propertyFile", "上传宽格式数据", accept = ".xlsx", buttonLabel = "浏览", placeholder = ".xlsx文件"),
                 fluidRow(
                   column(10, fileInput("additionalPropertyFile", "追加中药属性", accept = ".xlsx", buttonLabel = "浏览", placeholder = ".xlsx文件")),
                   column(2, tags$a(href = "/miner/www/herb_property_added.xlsx", target = "_blank", "示例", style = "margin-top: 40px; display: inline-block;"))
                 ),
                 fluidRow(
                   column(10, fileInput("efficacyFile", "追加中药功效", accept = ".xlsx", buttonLabel = "浏览", placeholder = ".xlsx文件")),
                   column(2, tags$a(href = "/miner/www/herb_property_added.xlsx", target = "_blank", "示例", style = "margin-top: 40px; display: inline-block;"))
                 ),
                 ...
                 hidden(
                   div(id = "visual_params",
                       ...
                       numericInput("label_size", "字体大小", value = 6, min = 1),
               ...
               mainPanel(
                 tabsetPanel(
                   tabPanel("归经", dataTableOutput("propertyGjTable")),
                   tabPanel("四气", dataTableOutput("propertySqTable")),
                   tabPanel("五味", dataTableOutput("propertyTasteTable")),
                   tabPanel("功效", dataTableOutput("efficacyTable")),
                   tabPanel("归经可视化",
                   ...
                   tabPanel("功效可视化",
                            ...
                            fluidRow(
                              class = "align-bottom-row",
                              column(3,
                                     div(class = "form-group",
                                         selectInput("cal_pal", "颜色方案",
                              ...
                              column(3,
                                     div(class = "form-group",
                                         numericInput("ignore", "ignore参数",
                                                      value = 0.03, min = 0.01, max = 0.99, step = 0.01)
                                     )
                              ),
                              column(3,
                                     div(class = "form-group",
                                         numericInput("lab_size", "字体大小",
                                                      value = 2.5, min = 1)
                                     )
                              ),
                              column(3,
                                     div(class = "form-group",
                                         downloadButton("downloadEfficacyPlot", "图片",
             ...
             sidebarLayout(
               sidebarPanel(
                 ...
                 div(class = "file-input-margin",
                     fileInput("arFile", "宽格式数据", accept = ".xlsx", buttonLabel = "浏览", placeholder = ".xlsx文件")
                 ),
                 textInput("supp_values", "支持度组合 (英文逗号)", value = "0.05, 0.07, 0.10, 0.12, 0.15, 0.2, 0.22, 0.25, 0.28, 0.3"),
                 textInput("conf_values", "置信度组合 (英文逗号)", value = "0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9"),
                 div(class = "btn-group-custom",
                 ...
                 tabsetPanel(
                   ...
                   tabPanel("探索可视化",
                            div(class = "tab-content",
                                ...
                                hidden(
                                  div(id = "params",
                                      selectInput("col_pal", "配色方案", choices = c("A", "B", "C", "D", "E", "F", "G", "H"), selected = "D")
                                  )
                                ),
                   ...
                   tabPanel("规则分布",
                            div(class = "tab-content param-group",
                                selectInput("distr_col_pal", "配色方案", choices = c("A", "B", "C", "D", "E", "F", "G", "H"), selected = "D"),
    ...
    tabPanel("处方相似度",
             sidebarLayout(
               sidebarPanel(
                 fileInput("simFile", "宽格式数据", accept = ".xlsx", buttonLabel = "浏览", placeholder = "选择.xlsx文件"),
                 ...
                 tabsetPanel(
                   ...
                   tabPanel("可视化",
                            ...
                            fluidRow(
                              ...
                              column(3,numericInput("heatmapWidth", "宽度 (cm)", value = 8, min = 1)),
                              column(3,numericInput("heatmapHeight", "高度 (cm)", value = 6, min = 1)),
                              style = "display: flex; align-items: flex-end; gap: 5px;")
                   )
                 )
               )
    ...
    tabPanel("聚类分析",
             sidebarLayout(
               sidebarPanel(
                 fileInput("clusterFile", "上传宽数据", accept = ".xlsx", buttonLabel = "浏览",placeholder = "选择一个.xlsx文件"),
                 ...
                 tabsetPanel(
                   ...
                   tabPanel("Phi聚类可视化",
                            ...
                            fluidRow(
                              ...
                              column(3,numericInput("phiPlotWidth", "宽度 (cm)", value = 8, min = 1)),
                              column(3,numericInput("phiPlotHeight", "高度 (cm)", value = 6, min = 1)),
                              style = "display: flex; align-items: flex-end; gap: 5px;")
                   )
                 )
               )
    ...
    tabPanel("共现网络",
             sidebarLayout(
               sidebarPanel(
                 fileInput("coocFile", "上传宽数据", accept = ".xlsx", buttonLabel = "浏览",placeholder = "选择一个.xlsx文件"),
    ...
    tabPanel("剂量分析",
             ...
             sidebarLayout(
               ...
               # 侧边栏：用于上传文件和按钮
               sidebarPanel(
                 fileInput("file1", "上传中药剂量数据", accept = c(".xlsx")),
                 hr(),
...
server <- function(input, output, session) {
  ...
  res_auth <- secure_server(
    check_credentials = check_credentials(
      "/srv/shiny-server/database.sqlite",
  ...
  add_herb_property <- reactiveVal(NULL)
  add_herb_efficacy <- reactiveVal(NULL)
  ...
  observeEvent(input$additionalPropertyFile, {
    add_herb_property(read.xlsx(input$additionalPropertyFile$datapath))
  })
  observeEvent(input$efficacyFile, {
    add_herb_efficacy(read.xlsx(input$efficacyFile$datapath))
  })

  # 计算逻辑
  observeEvent(input$calcProperty, {
    req(data_clean_wide_property())
    property <- calc_property(data_clean_wide_property(), added = add_herb_property())
    property_result(property)
    efficacy_result(calc_efficacy(data_clean_wide_property(), added = add_herb_efficacy()))
  })
  ...
  output$plotGjVisualization <- renderPlot({
    req(data_clean_wide_property())
    fig_guijing(data_clean_wide_property(),
                line_col = input$line_col,
                bac_col = input$bac_col,
                label.size = input$label_size,
                line.width = input$line_width,
                added = add_herb_property())
  })
  output$plotSqVisualization <- renderPlot({
    req(data_clean_wide_property())
    fig_siqi(data_clean_wide_property(),
             line_col = input$line_col,
             bac_col = input$bac_col,
             label.size = input$label_size,
             line.width = input$line_width,
  ...
  output$plotTasteVisualization <- renderPlot({
    req(data_clean_wide_property())
    fig_taste(data_clean_wide_property(),
              line_col = input$line_col,
              bac_col = input$bac_col,
              label.size = input$label_size,
              line.width = input$line_width,
  ...
  output$plotEfficacyVisualization <- renderPlot({
    req(data_clean_wide_property())
    fig_efficacy(data_clean_wide_property(),
                 cal_pal = input$cal_pal,
                 ignore = input$ignore,
                 lab_size = input$lab_size,
                 added = add_herb_efficacy())
  })

  # 下载处理
  output$downloadProperty <- downloadHandler(
    filename = function() { paste("中药属性功效-", Sys.Date(), ".xlsx", sep="") },
    content = function(file) {
      req(property_result())
      wb <- createWorkbook()
      addWorksheet(wb, "归经")
  ...
  output$downloadGjPlot <- downloadHandler(
    filename = function() { paste("归经-", Sys.Date(), ".", tolower(input$file_format), sep="") },
    content = function(file) {
      plot_obj <- fig_guijing(data_clean_wide_property(),
                              line_col = input$line_col,
                              bac_col = input$bac_col,
                              label.size = input$label_size,
                              line.width = input$line_width,
                              added = add_herb_property())
      ggsave(file, plot_obj, device = tolower(input$file_format),
             width = input$img_width_efficacy, height = input$img_height_efficacy,
             dpi = 600, units = "cm")
    }
  )
  output$downloadSqPlot <- downloadHandler(
    filename = function() { paste("四气-", Sys.Date(), ".", tolower(input$file_format), sep="") },
    content = function(file) {
      plot_obj <- fig_siqi(data_clean_wide_property(),
                           line_col = input$line_col,
                           bac_col = input$bac_col,
                           label.size = input$label_size,
                           line.width = input$line_width,
  ...
  output$downloadTastePlot <- downloadHandler(
    filename = function() { paste("五味-", Sys.Date(), ".", tolower(input$file_format), sep="") },
    content = function(file) {
      plot_obj <- fig_taste(data_clean_wide_property(),
                            line_col = input$line_col,
                            bac_col = input$bac_col,
                            label.size = input$label_size,
                            line.width = input$line_width,
  ...
  output$downloadEfficacyPlot <- downloadHandler(
    filename = function() { paste("功效-", Sys.Date(), ".", tolower(input$file_format), sep="") },
    content = function(file) {
      plot_obj <- fig_efficacy(data_clean_wide_property(),
                               cal_pal = input$cal_pal,
                               ignore = input$ignore,
                               lab_size = input$lab_size,
                               added = add_herb_efficacy())
  ...
  output$download_plot_dose <- downloadHandler(
    ...
    content = function(file) {
      ggsave(file, plot = dose_p(),
             device = tolower(input$img_format_dose),
